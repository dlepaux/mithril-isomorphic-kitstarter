    ...
    browserify:
      ...
      bundles:
        'index.js':
          ...
          # Any files watched by browserify won't be in brunch's regular pipeline. If you do anything before your javascripts are compiled, now's the time. 
          onBeforeBundle: (bundler) -> console.log 'onBeforeBundle'
          # Any files watched by browserify won't be in brunch's regular pipeline. If you do anything after your javascripts are compiled, now's the time. 
          onAfterBundle: (error, bundleContents) -> console.log 'onAfterBundle'
          # Any options to pass to `browserify`. `debug` will be set to `!production` if not already defined. `extensions` will be set to a proper list of `plugins.browserify.extensions` 
          instanceOptions: {}
    afterBrunch: [
